...
Gathering::Gathering(const CRayTraceDoc* const pDoc, node_type NodeType, const char* const Name)
	: Node(pDoc, NodeType, Name)
{
	m_Member = 0;
}

Gathering::Gathering(const Gathering& other) : Node(other)
{
	m_Member = other.m_Member;
	for (short i = 0; i < m_Member; i++)
		m_Node[i] = (BaseNode*)((Node*)other.m_Node[i])->MakeCopy();
}
...
void Gathering::Serialize(CArchive& ar)
{
	...
	else
	{
		...
		for ( i = 0; i < m_Member; i++ )
		{
			...
			switch ( id )
			{
			case SPHERE:	m_Node[i] = (BaseSphere*)	new Sphere(m_pDoc); 	break;
			case PLANE:		m_Node[i] = (BasePlane*)	new Plane(m_pDoc);		break;
			case PLUS:		m_Node[i] = (BasePlus*)		new Plus(m_pDoc);		break;
			case MINUS:		m_Node[i] = (BaseMinus*)	new Minus(m_pDoc);		break;
			case MULTIPLE:	m_Node[i] = (BaseMultiple*)	new Multiple(m_pDoc);	break;
			case CONE:		m_Node[i] = (BaseCone*)		new Cone(m_pDoc);		break;
			case CYLINDER:	m_Node[i] = (BaseCylinder*)	new Cylinder(m_pDoc);	break;
			case TORUS:		m_Node[i] = (BaseTorus*)	new Torus(m_pDoc);		break;
			case POLYGON:	m_Node[i] = (BasePolygon2*)	new Polygon2(m_pDoc);	break;
			case CUBE:		m_Node[i] = (BaseCube*)		new Cube(m_pDoc);		break;
			}
			((Node*)m_Node[i])->Serialize(ar);
		}
	}
...
void Gathering::Draw_Outline(CDC* pDC, CRayTraceView& rtv, const matrix& Matrix) const
{
	...
	if (pNodeBackup == (Node*)this) {
		for (int i = 0; i < m_Member; i++) {
			rtv.m_SelectedNode = (Node*)m_Node[i];
			((Node*)m_Node[i])->Draw_Outline(pDC, rtv, m);
		}
...
void Gathering::AddGeometry(LPDIRECT3DDEVICE9 pd3dDevice, CListGeometry& lstGeometry, CRayTraceView& rtv, const matrix& Matrix) const
{
	matrix m = Matrix * m_Matrix;
	for (int i = 0; i < m_Member; i++)
		((Node*)m_Node[i])->AddGeometry(pd3dDevice, lstGeometry, rtv, m);
...
bool Gathering::SetManipulatorAxis(CRayTraceView& rtv, CPoint point, const matrix& Matrix) const
{
	matrix m = Matrix * m_Matrix;
	for (int i = 0; i < m_Member; i++) {
		if (((Node*)m_Node[i])->SetManipulatorAxis(rtv, point, m))
			return true;
	}
	return false;
}
...
{
	for (short i = 0; i < m_Member; i++)
		((Node*)m_Node[i])->InsertItem(c, hParent);
	...
	for (i = 0; i < m_Member; i++) {
		if (m_Node[i] == pNode) {
			...
			for (j = i; j < m_Member; j++)
				m_Node[j] = m_Node[j+1];
			return TRUE;
		} else if (((Node*)m_Node[i])->Delete(pNode))
			return TRUE;
	}
	return FALSE;
}
...
void Gathering::SetDocument(const CRayTraceDoc* const pDoc)
{
	...
	for (int i = 0; i < m_Member; i++)
		((Node*)m_Node[i])->SetDocument(pDoc);
}
