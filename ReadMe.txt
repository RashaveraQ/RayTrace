========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : RayTrace
========================================================================


AppWizard が生成した RayTrace には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには RayTrace を構成している各ファイルの概要説明が含まれています。

RayTrace.h
    このファイルはアプリケーションの中心となるインクルードファイルです。この
    ファイルは他のプロジェクトの固有のインクルードファイル（Resource.h  も含
    みます） をインクルードし、また CRayTraceApp アプリケーションクラスを宣
    言します。

RayTrace.cpp
    このファイルは CRayTraceApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\RayTrace.ico
    ...
    ルです。このアイコンはリソース ファイル RayTrace.rc によってインクルード
    されます。
...
RayTraceDoc.h, RayTraceDoc.cpp - ドキュメント
    これらのファイルは CRayTraceDoc クラスを含みます。特別なドキュメント デ
    ータを付け加えたり、CRayTraceDoc::Serialize を使ったファイルのセーブや
    ロード機能をインプリメントしたいときはこのファイルを編集してください。

RayTraceView.h, RayTraceView.cpp - ビュー
    これらのファイルは CRayTraceView クラスを含みます。
    CRayTraceView オブジェクトは CRayTraceDoc オブジェクトを表示するために
    使います。

res\RayTraceDoc.ico
    このファイルは CRayTraceDoc クラスの MDI 子ウィンドウ用アイコンとして使
    われるアイコンファイルです。このアイコンはリソースファイル RayTrace.rc
    によってインクルードされます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) RayTrace.pch 
